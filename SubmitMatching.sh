...
for run in ${Runs[@]}; do
  ...
  for file in ${Files[@]}; do
    ...
    pyFile=$Prefix"_"$run"_"$file".pythia"$Suffix
    pyPath=$runDir"/"$pyFile
    if [ ! -f $pyPath ]; then
      continue
    fi
...
arg=$(echo -e "\($Nevnt,$Nfile,$ge,$mu,$py,$out\)")
